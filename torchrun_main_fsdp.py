...
def evaluate_model(
    ...
    # local_datasets_path = '/data/datasets/c4_en'
    # val_data = datasets.load_dataset(local_datasets_path, split='validation', streaming=True)
...
def main(args):
    ...
    if global_rank == 0:
        ...
        ## if use wandb sweep, then use the following code
        # config = wandb.config
        # args.lr = config.lr
        # if args.peft_model.lower() == "sltrain":
        #     args.rank = config.rank
        #     args.lora_alpha = config.lora_alpha
        #     args.sp_ratio = config.sp_ratio
        # if args.peft_model.lower() == "fourier_low_rank":
        #     args.rank = config.rank
        #     args.lora_alpha = config.lora_alpha
        #     args.n_freq = config.n_freq
    ...
    # ## use local dataset
    # local_dataset_path = '/data/datasets/c4_en'
    # data = datasets.load_dataset(local_dataset_path, split='train', streaming=True)
    ...
    # FSDP should be created before the optimizer
    if not args.single_gpu:

        model: LlamaForCausalLM = FSDP(
            model,
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        ## limit grad growth (only for splora_svd parameters)
        if args.gradient_control:
            for n, p in model.named_parameters():
                if 'lora' in n:
                    if p.grad is not None:
                        ...
                        if f"{n}_prev_norm" in grad_state:
                            prev_norm = grad_state[f"{n}_prev_norm"]
                            limiter = max(
                                grad_norm / (prev_norm + 1e-8),
                                1.001
                            ) / 1.001
                            
                            p.grad.data = grad / limiter
                            grad_state[f"{n}_prev_norm"] = grad_norm / limiter
                        else:
                            grad_state[f"{n}_prev_norm"] = grad_norm
        ...
        # 修改后的代码
        # 在FSDP中，不需要手动进行梯度累积，且梯度裁剪应在所有参数裁剪后聚合
        if args.grad_clipping != 0.0:
        ...
        if not layer_wise_flag:
            ...
            # 修改后的代码
            # with model.no_sync():  # no_sync() is needed for FSDP
                optimizer.step()
                scheduler.step()
                optimizer.zero_grad()
        ...
        if update_step % args.cycle_length == 0 and (args.peft_model.lower() == 'restart_lora') and update_step > args.warmup_steps:
    ...
    #     # 修改后的代码
    #     model.save_state_dict(current_model_directory)
    ...
    if global_rank == 0:
        ...
        # 保存模型状态
        torch.save(model.module.state_dict(), os.path.join(current_model_directory, 'model_state_dict.pt'))
