...
class ReLoRaModel(torch.nn.Module):
    ...
    def merge_and_reinit_2(self, optimizer = None, rand = False):
        for module in self.modules():
            if isinstance(module, ReLoRaLinear):
                module.merge_and_reinit_2(optimizer, rand)
...
class ReLoRaLinear(nn.Module):
    ...
    def merge_and_reinit(self, optimizer = None, rand = False):
        ...
        if self.proj_type:
            ...
            else: self.lora_B.weight.copy_(get_random_orthogonal_matrix(self.relora_config.r, WG.shape[1],device=self.device, dtype = WG.dtype))
            ...
            else: self.lora_A.weight.copy_(get_random_orthogonal_matrix(WG.shape[0], self.relora_config.r ,device=self.device, dtype = WG.dtype))
    ...
    #         with record_function("Step 1: Handle weight and grad"):
    #             # Step 1: 处理 weight 和 grad
    #             WG = self.weight.grad.detach().clone()
    #             self.weight.grad.zero_()
    #             self.weight.requires_grad = False
    #             self.weight.addmm_(self.lora_A.weight, self.lora_B.weight, alpha=-self.scale)
    ...
    #         if self.proj_type:
    #             with record_function("Step 2: Proj_type logic"):
    #                 # Step 2: proj_type 分支
    #                 B0 = self.lora_B.weight.detach().clone()
    #                 if not rand:
    #                     self.lora_B.weight.copy_(get_orthogonal_matrix(WG, self.relora_config.r, 'right'))
    #                 else:
    #                     self.lora_B.weight.copy_(get_random_orthogonal_matrix(self.relora_config.r, WG.shape[1], device = self.device, dtype=WG.dtype))

    #                 with record_function("Step 2.1: Optimizer state"):
    #                     # Step 2.1: Optimizer状态更新
    #                     if isinstance(optimizer, AdamW) and 'exp_avg' in optimizer.state[self.lora_A.weight]:
    #                         exp_avg = optimizer.state[self.lora_A.weight]['exp_avg'].detach().clone()
    #                         optimizer.state[self.lora_A.weight]['exp_avg'] = (exp_avg @ B0) @ self.lora_B.weight.T

    #                 with record_function("Step 2.2: Zero-out and grad"):
    #                     # Step 2.2: Zero-out lora_A.weight and计算梯度
    #                     self.lora_A.weight.zero_()
    #                     self.lora_A.requires_grad_(True)
    #                     self.lora_A.weight.grad = torch.zeros_like(self.lora_A.weight)
    #                     self.lora_A.weight.grad.copy_(-self.scale * torch.mm(WG, self.lora_B.weight.T))
    #         else:
    #             with record_function("Step 3: Non-proj_type logic"):
    #                 # Step 3: 非 proj_type 分支
    #                 A0 = self.lora_A.weight.detach().clone()
    #                 if not rand:
    #                     self.lora_A.weight.copy_(get_orthogonal_matrix(WG, self.relora_config.r, 'left'))
    #                 else:
    #                     self.lora_A.weight.copy_(get_random_orthogonal_matrix(WG.shape[0], self.relora_config.r, device = self.device, dtype=WG.dtype))

    #                 with record_function("Step 3.1: Optimizer state"):
    #                     # Step 3.1: Optimizer状态更新
    #                     if isinstance(optimizer, AdamW) and 'exp_avg' in optimizer.state[self.lora_B.weight]:
    #                         exp_avg = optimizer.state[self.lora_B.weight]['exp_avg'].detach().clone()
    #                         optimizer.state[self.lora_B.weight]['exp_avg'] = self.lora_A.weight.T @ (A0 @ exp_avg)

    #                 with record_function("Step 3.2: Zero-out and grad"):
    #                     # Step 3.2: Zero-out lora_B.weight and计算梯度
    #                     self.lora_B.weight.zero_()
    #                     self.lora_B.requires_grad_(True)
    #                     self.lora_B.weight.grad = torch.zeros_like(self.lora_B.weight)
    #                     self.lora_B.weight.grad.copy_(-self.scale * torch.mm(self.lora_A.weight.T, WG))
