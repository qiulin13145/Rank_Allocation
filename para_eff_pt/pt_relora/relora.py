...
class ReLoRaModel(torch.nn.Module):
    def __init__(
        ...
        *,
        ...
        if r <= 0:
            raise ValueError(
                "r must be positive. If you want r == 0, use the original model."
            )
...
class ReLoRaLinear(nn.Module):
        ...
        *,
        ...
        if lora_only:
            ...
            if quantize is None:
                ...
                logger.warning(
                    "Int8 currently does not support merge_and_reinit! It will fail"
                )
    ...
    def merge_and_reinit(self):
        if self.lora_only:
            print(
                "WARNING: Skipping merge and reinit, because only lora parameters are used"
            )
            return

        if not self.quantize:
            self.weight.data += (
                self.lora_B.weight @ self.lora_A.weight * self._post_lora_scale()
            )
        elif self.quantize == "4bit":
            self.weight: bnb.nn.Params4bit
            _weight_fp = torch.empty(
                self.weight.data.shape,
                dtype=self.lora_B.weight.dtype,
                device=self.weight.data.device,
            )
            bnbF.dequantize_4bit(
                self.weight.data, self.weight.quant_state, out=_weight_fp
            )
            _weight_fp += (
                self.lora_B.weight @ self.lora_A.weight * self._post_lora_scale()
            )
            ...
            bnbF.dequantize_blockwise(
                self.weight.data, self.self.lora_B.weight.dtype, out=_weight_fp
            )
            _weight_fp += (
                self.lora_B.weight @ self.lora_A.weight * self._post_lora_scale()
            )
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.quantize == "4bit":
            result = bnb.matmul_4bit(
                x, self.weight.t(), bias=self.bias, quant_state=self.weight.quant_state
            )
        elif self.quantize == "8bit":
            result = bnb.matmul(
                x, self.weight.t(), bias=self.bias, quant_state=self.weight.quant_state
