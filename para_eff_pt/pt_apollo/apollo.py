...
class Apollo_AdamW(Optimizer):
    ...
    def _initialize_projector(self, group, state):
        ...
        elif group["proj"] == "full":
            #print("Apollo: group[proj] = ", group["proj"], " group[proj_type] ", group["proj_type"] )
            return GradientProjector(
                group["rank"],
                update_proj_gap=group["update_proj_gap"],
                scale=group["scale"],
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if "rank" in group:
                    ...
                    name = self.param_to_name.get(p, None)
                    if name is not None:
                        #print(f"Parameter name: {name}")
                        gradnorms.append((  name , torch.norm(norm_grad),  torch.norm(scaled_grad)))   # name, gradnorm before NL, gradnorm after NL
