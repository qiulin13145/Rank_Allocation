...
def main(args):
    ...
    if not args.sweep_seed:
        print(logger.info(f"setting based on args seed {args.seed}"))
    ...
    if global_rank == 0:
        ...
        if args.sweep_seed:
            # SET SEED AGAIN BASED ON CONFIG
            args.seed = config.seed
            torch.manual_seed(args.seed)
            np.random.seed(args.seed)
            random.seed(args.seed)
            logger.info(f"using seed {args.seed} from config.")        
        ...
        if args.peft_model.lower() == "sltrain":
            ...
            # assuming that if we use cosine_quick_recovery and sltrain we are performing restarts ...
            if args.scheduler == "cosine_quick_recovery":
                args.cycle_length = config.cycle_length              
        elif args.peft_model.lower() == "restart_lora":
            args.rank = config.rank
            args.lora_alpha = config.lora_alpha 
            args.cycle_length = config.cycle_length 
        elif args.peft_model.lower() == "galore":
            args.rank = config.rank
            args.update_proj_gap = config.update_proj_gap
            # !!!
            #args.galore_scale = config.galore_scale
        elif args.peft_model.lower() == "fira":
            args.rank = config.rank
            args.update_proj_gap = config.update_proj_gap            
            ...
            args.rank = config.rank
            args.lora_alpha = config.lora_alpha
        elif args.peft_model.lower() == "low-rank" and not ("loro" in args.optimizer.lower()) : # low-rank
            args.rank = config.rank
            args.lora_alpha = config.lora_alpha
        
        elif "loro" in args.optimizer.lower(): # LORO
            args.rank = config.rank
            args.cycle_length = config.cycle_length 
    ...
    if global_rank == 0:
        ...
        object_list = [None] * len(keys)
    
    dist.broadcast_object_list(object_list, src=0)
    
    if global_rank != 0:
        for key, value in zip(keys, object_list):
    ...
    if ("galore" in args.optimizer.lower()) or ("fira" in args.optimizer.lower()):
        ...
        for module_name, module in model.named_modules():
            ...
            if not ((isinstance(module, nn.Linear)) or (module.__class__.__name__ == 'SpLoRaLinear')):
            ...
            if module.__class__.__name__ == 'SpLoRaLinear':
